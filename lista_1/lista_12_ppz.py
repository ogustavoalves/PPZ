...
def inicio_final(a, b):
  ...
  def str_div (x):
    if len(x) % 2 != 0:
      x_inicio = x[0:len(x)//2+1]
      x_final = x[len(x)//2+1:]
    else:
      x_inicio = x[0:len(x)//2]
      x_final = x[len(x)//2:]
    
    return x_inicio, x_final
  
  a_inicio, a_final = str_div(a)
  b_inicio, b_final = str_div(b)
...
def zf(n):
  ...
  for x in range(len(n_reversed)):
    if n_reversed[x] == '0': count += 1
    else: break
...
def conta2(n):
  ...
  for i in range(n):
    n_list.append(i)
    n_list_str = "".join(str(n_list))
    num_2 = n_list_str.count("2")
  return num_2
